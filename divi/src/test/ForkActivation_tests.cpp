...
public:
  ...
  AddBlock()
  {
    ...
    blocks.emplace_back(new CBlockIndex());
    blocks.back()->pprev = pprev;
    blocks.back()->nVersion = 4;
...
BOOST_AUTO_TEST_CASE(activationByTimestamp)
{
  ...
  BOOST_CHECK_EQUAL(CurrentState().IsActive(Fork::TestByTimestamp), true);
  ...
  for (int i = 0; i < 20; ++i)
    AddBlock()->nTime = 1000000000;
  AddBlock()->nTime = 999999999;
  BOOST_CHECK_EQUAL(CurrentState().IsActive(Fork::TestByTimestamp), false);
}
